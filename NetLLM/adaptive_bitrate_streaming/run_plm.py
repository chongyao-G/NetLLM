...
def load_model(args, model, model_dir):
    if args.rank > 0:
        ...
        model.modules_except_plm.load_state_dict(torch.load(os.path.join(model_dir, 'modules_except_plm.bin'), map_location=torch.device('cuda:0')))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--trace-num', help='number of traces. if set to -1, use all traces in the trace dir.', type=int, default=100)#from 100 to -1
